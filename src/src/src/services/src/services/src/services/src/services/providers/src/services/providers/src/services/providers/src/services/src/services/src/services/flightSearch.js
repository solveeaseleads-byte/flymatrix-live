import {
  searchDuffelFlights
} from "./providers/duffel.js";

import {
  normalizeDuffelSearch
...
export async function searchFlights(params) {
  ...
  if (!validIata(origin)) {
    throw new Error(
      "Origin must be a valid three-letter IATA code."
    );
  }

  if (!validIata(destination)) {
    throw new Error(
      "Destination must be a valid three-letter IATA code."
    );
  }

  if (!validDate(departureDate)) {
    throw new Error(
      "Departure date must use YYYY-MM-DD."
    );
  }
