...
const BASE_URL =
  config.providers.flightApiBaseUrl ||
  "https://api.travelpayouts.com";

/**
 * Travelpayouts Data API (cache-based prices).
 * Docs: https://support.travelpayouts.com/hc/en-us/articles/203956163
 *
 * Note: this returns cached prices from Aviasales search history
 * (refreshed periodically), not a live real-time quote. The
 * real-time Flight Search API requires separate approval and a
 * 50k MAU minimum - swap this provider out for that later without
 * touching the router or route.
 */
export async function searchTravelpayouts({
  ...
  if (!response.ok) {
    throw new Error(
      `Travelpayouts request failed: ${response.status}`
    );
  }
