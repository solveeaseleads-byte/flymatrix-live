...
/*
 * Resolve the appropriate active affiliate program.
 */
router.get("/resolve", async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error:
        "Unable to resolve booking partner."
    });
  }
});
 ...
 *
 * The frontend should call this before opening
 * the partner URL.
 */
  ...
  try {
    ...
    if (!affiliateProgramId) {
      return res.status(400).json({
        success: false,
        error:
          "Affiliate program ID is required."
      });
    }
