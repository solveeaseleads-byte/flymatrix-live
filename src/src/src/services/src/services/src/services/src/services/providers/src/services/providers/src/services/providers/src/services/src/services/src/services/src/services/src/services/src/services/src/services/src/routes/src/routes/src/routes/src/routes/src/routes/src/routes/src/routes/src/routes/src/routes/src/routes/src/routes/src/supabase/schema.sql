...
create index if not exists idx_affiliate_category_market
on affiliate_programs(category, market);

create index if not exists idx_affiliate_active
on affiliate_programs(active);
...
on fare_alerts(origin, destination);

create index if not exists idx_alerts_status
on fare_alerts(status);
...
on flight_search_events(origin, destination);
